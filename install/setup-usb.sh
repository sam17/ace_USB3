...
if [ -f "$FILE" ] ; then
	echo "\nChecking if $FILE must be updated"
...
###BASLER_END #####################################################\" >> $FILE"
	...
	echo "
Couldn't locate the usb-ids database. This is no big problem, you will only miss descriptions for Basler devices when calling lsusb."
fi
